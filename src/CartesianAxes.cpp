...
namespace sf
{
const sf::Vector2f CartesianAxes::DIMENSION{800.0, 600.0}; /**< dimension of Cartesian chart */
const sf::Vector2f CartesianAxes::MARGIN{40.0, 20.0}; /**< margin between plot and chart dimension */
...
void CartesianAxes::drawLegend(sf::RenderTarget& target, sf::RenderStates states) const
{
    ...
    const int x_precision = std::abs(std::min(0.0, std::log10(x_step) - 2.0));
    ...
    const int y_precision = std::abs(std::min(0.0, std::log10(y_step) - 2.0));
...
*/
void CartesianAxes::setRangeX(const std::pair<double, double>& x_range)
{
    x_range_ = x_range;
}
...
*/
void CartesianAxes::setRangeY(const std::pair<double, double>& y_range)
{
    y_range_ = y_range;
}
...
*/
std::pair<double, double> CartesianAxes::getRangeX() const
{
    return x_range_;
}
...
*/
std::pair<double, double> CartesianAxes::getRangeY() const
{
    return y_range_;
}
